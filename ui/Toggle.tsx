...
export default function Toggle({ title, setValue, value=false, disabled=false, className="" }) {
    ...
    return (
        <label htmlFor="default-toggle" className={`inline-flex relative items-center ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'} ${className}`}>
                ...
                />
                <span onClick={() => !disabled && handleClick()} className="ml-3 text-sm font-medium text-gray-900 select-none dark:text-gray-300">{ title }</span>
        </label>
    )
}