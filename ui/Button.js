...
export default function Button({ title, small=false, color="primary", onClick, className="", }) {
    let colorClasses
    ...
    return (
        <button 
            type="button" 
            onClick={onClick}
            className={`focus:ring-4 font-medium ${sizeClasses} focus:outline-none ${colorClasses} ${className}`}>{title}</button>
    )
  }