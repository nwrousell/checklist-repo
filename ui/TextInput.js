export default function TextInput({ title, inputRef=null, placeholder=null, error=false, setValue, required=false, className="" }){
    ...
    return (
        <div className={className}>
            ...
            <input ref={inputRef} onChange={(e) => setValue(e.target.value)} type="text" id="first_name" className={`${inputClasses} min-w-max`} placeholder={placeholder || `${title}...`} required={required} />
        </div>
    )
}