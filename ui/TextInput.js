export default function TextInput({ title, initialValue="", inputRef=null, placeholder=null, error=false, setValue, required=false, className="", password=false }){
    ...
    return (
        <div className={className}>
            ...
            <input ref={inputRef} defaultValue={initialValue} onChange={(e) => setValue(e.target.value)} type={password ? 'password' : 'text'} id={`text-${title.replaceAll(" ", "-")}`} className={`${inputClasses}`} placeholder={placeholder || `${title}...`} required={required} />
        </div>
    )
}